...
micBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!text) {
      textInput.value =  sttJson?.error ? `${TextRobot}ไม่เข้าใจเสียง: ${sttJson.error}` : 'RUS Bot : ไม่พบคำพูด';
    } else {
      textInput.value = "User :" + text;
      await sendToAI(text);
    }
  } catch (err) {
    textInput.value = TextSystem + String(err)
  } finally {
...
async function sendToAI(sentence) {
  ...
  try {
    ...
    textInput.value =  TextRobot + answer || '';
